...
*Top panel: absolute tokens — bar length is the total generated across the
analyzed jobs, so bars differ by how much history each run had (G/G's 12
sacct-verified jobs vs R/G's 2). Bottom panel: the same totals normalized per
run. Destroyed totals = kills × per-kill loss; B/B's kill count is
counter-derived and unverified, so its destroyed total is an upper bound.
Plot source: `rollout_bank_design_assets/bank_kill_waste_overall.py`.*
