...
class UniformLatencyInterface(ReturnsRaw):
    ...
    async def base_generate(self, request):
        ...
        delay = self._rng.uniform(self.lo, self.hi)
...
class SimAgent(GroupedRolloutGenerator):
...
class ModeResult:
    ...
    engine_utilization: float | None  # busy slot-seconds / (slots * wall); None if unlimited
...
async def run_mode(submission, consumption, cfg: SimConfig) -> ModeResult:
    ...
    try:
        async for group in agen:
            ...
            group_delays = iface.delays_by_prompt[f"t{int(group[0].problem_id)}"]
            ...
            if cfg.streaming and len(groups) >= target_groups:
                break
            if batch_completed:
                # The trainer runs its step; the pipeline keeps whatever
                # generation its gate slots allow running in the background.
    ...
    return ModeResult(
        ...
        engine_utilization=(
            iface.busy_seconds / (cfg.engine_slots * total_wall) if cfg.engine_slots else None
        ),
...
def write_html_report(results, cfg: SimConfig, path, subtitle=None):
    ...
    train_desc = f", {cfg.train_time:g}s trainer pause per batch" if cfg.train_time else ""
...
def main():
    ...
    print(
        ...
        f"train_time={cfg.train_time}s, seed={cfg.seed}"
    )
