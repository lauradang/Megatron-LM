...
class RolloutBank:
    ...
    # ------------------------------------------------------------- lifecycle
    @property
    def collection_iteration(self) -> Optional[int]:
        """Return the open collection iteration, or None while the bank is closed."""
        return self._collection_iter if self._seg_dir is not None else None
    ...
    def checkpoint_preserving_collection(self, iteration: int) -> None:
        """Compact through ``iteration`` without leaving the live writer closed."""
        active_collection = self.collection_iteration
        self.checkpoint(iteration)
        if active_collection is not None:
            self.set_collection(active_collection)
